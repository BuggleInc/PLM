	...
	public void solve() {
		...
		while (true) {
			...
			if (sorted) { // we are done, no need to continue
				if (debug > 0) 
					Logger.debug("It's sorted now. Get out of here\n");
				break;
			}
			...
			if (maxupside != -1) { // Case 1. 
				if (maxupside == maxPos) { // Case 1.C
					if (debug > 0) 
						Logger.debug("Case 1.C; maxupsidePos = "+maxupsidePos+", maxupside = "+maxupside);
					...
					if (pPlus1 > maxupsidePos) {
						if (debug > 0) 
							Logger.debug("Case 1.A; maxupsidePos = "+maxupsidePos+", maxupside = "+maxupside+", pPlus1 = "+pPlus1);
						flip(pPlus1+1);showStack(true);
						flip(pPlus1-maxupsidePos);
					} else {
						if (debug > 0) 
							Logger.debug("Case 1.B; maxupsidePos = "+maxupsidePos+", maxupside = "+maxupside+", pPlus1 = "+pPlus1);						
						flip(maxupsidePos+1);
				...
				if (reverted) {
					...
					for (int radius=maxPos; radius > 0; radius--) {
						...
						if (pPlus1!=-99 && pPlus1<p) { // we've got the larger p such that p+1 is above p and both are upsideof
							if (debug > 0) 
								Logger.debug("Case 2.A; p="+p+", radius="+radius+", pPlus1="+pPlus1);
							flip(p+1);
							if (pPlus1!=0)
								flip(pPlus1+1);
