...
public class InterleavedHanoi extends ExerciseTemplated {
  ...
  public InterleavedHanoi(Lesson lesson)
  {
    ...
    HanoiWorld w;
    w = new HanoiWorld("solve(0,1,2,3)", new Integer[] {7, 6, 5, 4, 3, 2, 1}, new Integer[] {7, 6, 5, 4, 3, 2, 1}, new Integer[0], new Integer[0]);
    ...
    w = new HanoiWorld("solve(0,2,3,1)", new Integer[] {6, 5, 4, 3, 2, 1}, new Integer[0], new Integer[] {6, 5, 4, 3, 2, 1}, new Integer[0]);
    for (int i = 0; i < w.getSlotSize(0); i++)
    ...
    w = new HanoiWorld("solve(0,3,1,2)", new Integer[] {5, 4, 3, 2, 1}, new Integer[0], new Integer[0], new Integer[] {5, 4, 3, 2, 1});
    for (int i = 0; i < w.getSlotSize(0); i++)
      w.setColor(0, i, Color.white);
    ...
    myWorlds = Arrays.stream(myWorlds).map(s -> s.replaceEntities(InterleavedHanoiEntity::new)).toList().toArray(new HanoiWorld[0]);
    setup(myWorlds);
  }
}
