...
public class LightBotWorld extends plm.universe.GridWorld implements Iterable<LightBotWorldCell> {
  ...
  protected LightBotWorldCell newCell(int x, int y) { return new LightBotWorldCell(this, x, y); }
   ...
   * @param world2
   */
  public LightBotWorld(LightBotWorld world2) { super(world2); }
  ...
  @Override public WorldView getView()
  {
    //		return new LightBotWorldView2D(this);
    return new LightBotWorldViewIsometric(this);
  }
  @Override public ImageIcon getIcon() { return ResourcesCache.getIcon("img/world_lightbot.png"); }
  ...
  public class CellIterator implements Iterator<LightBotWorldCell> {
    ...
    @Override public boolean hasNext() { return x <= LightBotWorld.this.sizeX - 1 && y <= LightBotWorld.this.sizeY - 1; }

    @Override public LightBotWorldCell next()
    {
    ...
    @Override public void remove() { throw new RuntimeException("Method not implemented (and not implementable"); }
  }

  public void setHeight(int x, int y, int h) { ((LightBotWorldCell)getCell(x, y)).setHeight(h); }

  public void addLight(int x, int y) { ((LightBotWorldCell)getCell(x, y)).addLight(); }

  public void removeLight(int x, int y) { ((LightBotWorldCell)getCell(x, y)).removeLight(); }
