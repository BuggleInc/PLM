...
public class LightBotWorldView2D extends WorldView {
  ...
  public LightBotWorldView2D(World w) { super(w); }
  ...
  @Override public void paintComponent(Graphics g)
  {
    ...
    double ratio = Math.min(((double)getWidth()) / (tw.getWidth() * LightBotWorldView2D.CELL_WIDTH),
                            ((double)getHeight()) / (tw.getHeight() * LightBotWorldView2D.CELL_WIDTH));
    g2.translate(Math.abs((getWidth() - ratio * tw.getWidth() * LightBotWorldView2D.CELL_WIDTH) / 2.),
                 Math.abs((getHeight() - ratio * tw.getHeight() * LightBotWorldView2D.CELL_WIDTH) / 2.));
  ...
  private void drawWorld2D(Graphics2D g)
  {
    ...
    for (int x = 0; x < w.getWidth(); x++) {
      for (int y = 0; y < w.getHeight(); y++) {
        ...
        g.fill(new Rectangle2D.Double(x * LightBotWorldView2D.CELL_WIDTH, y * LightBotWorldView2D.CELL_WIDTH, LightBotWorldView2D.CELL_WIDTH,
                                      LightBotWorldView2D.CELL_WIDTH));
      }
    }
  ...
  private void drawLight2D(Graphics2D g, GridWorldCell cell, boolean lightOn)
  {
    ...
    g.fill(new Arc2D.Double(cell.getX() * LightBotWorldView2D.CELL_WIDTH + 0.1 * LightBotWorldView2D.CELL_WIDTH,
                            cell.getY() * LightBotWorldView2D.CELL_WIDTH + 0.1 * LightBotWorldView2D.CELL_WIDTH, 0.8 * LightBotWorldView2D.CELL_WIDTH,
                            0.8 * LightBotWorldView2D.CELL_WIDTH, 0, 360, Arc2D.OPEN));
  }
  ...
  private void drawBot2D(Graphics2D g, LightBotEntity bot)
  {
    ...
    g.fill(new Arc2D.Double((cx - 0.25) * LightBotWorldView2D.CELL_WIDTH, (cy + 0.1) * LightBotWorldView2D.CELL_WIDTH, 1.5 * width, 1.5 * height, 60, 60,
                            Arc2D.PIE));
  }
}
