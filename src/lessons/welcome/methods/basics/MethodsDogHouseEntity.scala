...
class MethodsDogHouseEntity extends SimpleBuggle {
	...
	override def left() {
	  ...
	  if (savedLine == -1) {
	    ...
	    var offset = Game.getInstance().getCurrentLesson().getCurrentExercise().asInstanceOf[Exercise].getSourceFile(Game.getInstance().programmingLanguageManager.SCALA, 0).getOffset()
