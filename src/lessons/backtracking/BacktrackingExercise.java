...
public abstract class BacktrackingExercise extends ExerciseTemplated {
  public BacktrackingExercise(Lesson lesson) { super(lesson); }
  protected void setup(World[] ws, BacktrackingEntity solver)
  {
    ...
    try {
      ...
      newSourceFromFile(Game.getInstance().programmingLanguageManager.JAVA, this.tabName, solver.getClass().getCanonicalName(), "java");
  ...
  protected void newBestSolution(BacktrackingPartialSolution sol) { ((BacktrackingWorld)getWorlds(WorldKind.CURRENT).get(0)).newBestSolution(sol); }
}
