...
public class ShortestPathMazeEntity extends plm.universe.bugglequest.SimpleBuggle implements lessons.maze.shortestpath.ShortestPathMazeEntityPrimitives {
  ...
  @Override public int getIndication(int x, int y)
  {
    BuggleWorldCell c = ((BuggleWorld)world).getCell(x, y);
    if (c.hasContent())
  ...
  // tools functions
  public boolean hasRightWall(int x, int y) { return hasLeftWall((x + 1) % getWorldWidth(), y); }

  public boolean hasBottomWall(int x, int y) { return hasTopWall(x, (y + 1) % getWorldHeight()); }
