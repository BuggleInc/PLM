...
public class TurtleWorld extends World {
  ...
  public TurtleWorld(String name) { super(name); }
  ...
  @Override public TurtleWorld replaceEntities(Supplier<? extends Entity> newEntitySupplier) { return (TurtleWorld)super.replaceEntities(newEntitySupplier); }

  public void addSizeHint(int x1, int y1, int x2, int y2) { addSizeHint(x1, y1, x2, y2, null); }
  ...
  public Iterator<Shape> shapes() { return shapes.iterator(); }

  public double getHeight() { return height; }
  public double getWidth() { return width; }

  @Override public EntityControlPanel getEntityControlPanel() { return new TurtleButtonPanel(); }

  @Override public WorldView getView() { return new TurtleWorldView(this); }
  @Override public ImageIcon getIcon() { return ResourcesCache.getIcon("img/world_turtle.png"); }

  // TODO implement world IO

  @Override public String toString()
  {
    String res = "TurtleWorld: name=" + getName() + ", size=" + width + "x" + height + ", parameters: " + parameters + ", shapes=[";
    synchronized (shapes) {
            ...
            if (l1.sameSlope(l2)) { // We cannot have inverted slopes because the extremities within a line are sorted
              if ((Line.doubleEqual(l1.x1, l2.x1) && Line.doubleEqual(l1.y1, l2.y1)) || // Same start. Keep the longer
                ...
                if (l1.getLength() > l2.getLength()) {
                  rmIdx = j;
                  //									System.out.println("1a: Kill "+shapes.get(j)+" because of
                  //"+shapes.get(i));
                } else {
                  rmIdx = i;
                  //									System.out.println("1b: Kill "+shapes.get(i)+" because of
                  //"+shapes.get(j));
                }
                shapes.remove(rmIdx);
                if (i >= rmIdx && rmIdx > 0)
                ...
                if (Line.doubleWithin(l1.x1, l2.x1, l2.x2) && Line.doubleWithin(l1.x2, l2.x1, l2.x2) && Line.doubleWithin(l1.y1, l2.y1, l2.y2) &&
                    Line.doubleWithin(l1.y2, l2.y1, l2.y2)) {
                  ...
                  if (i > 0)
                    i--;
                  changedSomething = true;
                } else if (Line.doubleWithin(l2.x1, l1.x1, l1.x2) && Line.doubleWithin(l2.x2, l1.x1, l1.x2) && Line.doubleWithin(l2.y1, l1.y1, l1.y2) &&
                           Line.doubleWithin(l2.y2, l1.y1, l1.y2)) {
      ...
      synchronized (other.shapes) {
        ...
        for (int i = 0; i < other.shapes.size(); i++)
          if (!other.shapes.get(i).equals(shapes.get(i)))
            sb.append(Game.i18n.tr("  {0} (got {1} instead of {2})\n", ((Shape)other.shapes.get(i)).diffTo(shapes.get(i)), other.shapes.get(i), shapes.get(i)));
      }
    }
    return sb.toString();
  }
...
class ShapeComparator implements Comparator<Shape> {
  ...
  @Override public int compare(Shape s1, Shape s2)
  {
