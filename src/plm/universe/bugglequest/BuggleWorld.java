...
public class BuggleWorld extends GridWorld {
  ...
  @Override public BuggleWorldView getView() { return new BuggleWorldView(this); }
  @Override public EntityControlPanel getEntityControlPanel() { return new BuggleButtonPanel(); }
  @Override public ImageIcon getIcon() { return ResourcesCache.getIcon("img/world_buggle.png"); }

  public boolean easter = false;
  /* IO related */
  @Override public boolean haveIO() { return true; }

  @Override public BuggleWorld replaceEntities(Supplier<? extends Entity> newEntitySupplier) { return (BuggleWorld)super.replaceEntities(newEntitySupplier); }
  ...
  public World readFromFile(String path, String classname, BuggleWorld res) throws IOException, BrokenWorldFileException
  {
    ...
    if (path.endsWith(".map"))
      System.err.println(Game.i18n.tr(
          "{0}: The path to the map on disk should not include the .map extension (or it won''t work in jarfiles). Please fix your exercise.", path));
    ...
    if (line == null)
      throw new BrokenWorldFileException(Game.i18n.tr("{0}.map: this file does not seem to be a serialized BuggleWorld (the file is empty).", path));

    Pattern p = Pattern.compile("^" + classname + ": ");
    Matcher m = p.matcher(line);
    if (!m.find())
      throw new RuntimeException(Game.i18n.tr("{0}.map: This file does not seem to be a serialized BuggleWorld (malformated first line: {1}).", path, line));
    name = m.replaceAll("");
    ...
    do {
      ...
      if (buggleMatcher.matches()) {
        ...
        if (x < 0 || x > width || y < 0 || y > height)
          throw new BrokenWorldFileException(Game.i18n.tr("Cannot put a buggle on coordinate {0},{1}: that''s out of the world.", x, y));
        ...
        if (dirName.equalsIgnoreCase("north"))
          ...
          throw new BrokenWorldFileException(Game.i18n.tr("Invalid buggle''s direction: {0}", buggleMatcher.group(3)));

        Color color;
        try {
          color = ColorMapper.name2color(buggleMatcher.group(4));
        } catch (InvalidColorNameException e) {
          throw new BrokenWorldFileException(Game.i18n.tr("Invalid buggle''s color name: {0}", buggleMatcher.group(4)));
        }
        Color brushColor;
        try {
          brushColor = ColorMapper.name2color(buggleMatcher.group(5));
        } catch (InvalidColorNameException e) {
          throw new BrokenWorldFileException(Game.i18n.tr("Invalid buggle''s color name: {0}", buggleMatcher.group(5)));
        }
        ...
        if (haveBaggle.equals("haveBaggle"))
          ...
          throw new BrokenWorldFileException("Broken file, invalid buggle carrying information '" + haveBaggle +
                                             "': A buggle can either carry a baggle (haveBaggle) or not (noBaggle).");
        ...
        if (x < 0 || x > width || y < 0 || y > height)
          throw new BrokenWorldFileException(Game.i18n.tr("Cannot define a cell on coordinate {0},{1}: that''s out of the world.", x, y));
        ...
        if (!baggleFlag.equalsIgnoreCase("baggle") && !baggleFlag.equalsIgnoreCase("nobaggle"))
          throw new BrokenWorldFileException(Game.i18n.tr("Expecting ''baggle'' or ''nobaggle'' but got {0} instead.", baggleFlag));

        if (!topWallFlag.equalsIgnoreCase("topwall") && !topWallFlag.equalsIgnoreCase("notopwall"))
          throw new BrokenWorldFileException(Game.i18n.tr("Expecting ''topwall'' or ''notopwall'' but got {0} instead.", topWallFlag));

        if (!leftWallFlag.equalsIgnoreCase("leftwall") && !leftWallFlag.equalsIgnoreCase("noleftwall"))
          throw new BrokenWorldFileException(Game.i18n.tr("Expecting ''leftwall'' or ''noleftwall'' but got {0} instead.", leftWallFlag));
        ...
        if (baggleFlag.equalsIgnoreCase("baggle"))
          try {
            ...
            throw new BrokenWorldFileException(
                Game.i18n.tr("The cell {0},{1} seem to be defined more than once. At least, there is two baggles here, which is not allowed.", x, y));
          }
        ...
        throw new BrokenWorldFileException(Game.i18n.tr("Parse error. I was expecting a cell or a buggle description but got: {0}", line));
      }
  ...
  @Override public void writeToFile(BufferedWriter writer) throws IOException
  {
    ...
    for (int x = 0; x < getWidth(); x++) {
      for (int y = 0; y < getHeight(); y++) {
        ...
        if ((cell.getColor() != Color.white) || cell.hasBaggle() || cell.hasLeftWall() || cell.hasTopWall() || cell.hasContent()) {
  ...
  public BuggleWorldCell getSelectedCell() { return selectedCell; }
  public void setSelectedCell(int x, int y) { selectedCell = getCell(x, y); }
  public void unselectCell() { selectedCell = null; }

  /* adapters to the cells */
  public BuggleWorldCell getCell(int x, int y) { return (BuggleWorldCell)super.getCell(x, y); }
  public void setColor(int x, int y, Color c) { getCell(x, y).setColor(c); }
  public void addContent(int x, int y, String string) { getCell(x, y).addContent(string); }
  public void addBaggle(int x, int y) { getCell(x, y).baggleAdd(); }
