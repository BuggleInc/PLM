...
public class SortingWorldView extends WorldView {
   ...
   */
  private int getValueColor(int value, int valueCount) { return (int)((((float)value) / ((float)valueCount)) * 255.); }

  /** Returns if we must use the state view ( else we must use the chrono view ) */
  protected boolean isUseStateView() { return useStateView; }
  protected void setUseStateView(boolean useStateView) { this.useStateView = useStateView; }
