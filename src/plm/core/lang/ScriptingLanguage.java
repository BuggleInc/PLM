...
public abstract class ScriptingLanguage extends ProgrammingLanguage {
	...
	public List<Entity> mutateEntities(Exercise exo, List<Entity> olds, StudentOrCorrection whatToMutate) {
          ...
          try {
            ...
            if (engine == null)
              throw new RuntimeException(Game.i18n.tr(
                  "No ScriptEngine for {0}. Please check your classpath and similar settings.", getLang()));
            ...
            if (ent.getScript(this) == null) {
              System.err.println(Game.i18n.tr(
                  "No {0} script source for entity {1}. Please report that bug against the PLM.", this, ent));
              return;
            }
            setupEntityBindings(
                ent); // Python wants to add extra definitions to intercept I/O
            ...
            if (!handleLangException(e, ent, progress)) {
              System.err.println(
                  Game.i18n.tr("Received a ScriptException that does not come from the {0} language.\n", getLang()) +
                  e);
              e.printStackTrace();
            }
            ...
            String msg = Game.i18n.tr(
                "Script evaluation raised an exception that is not a ScriptException but a {0}.\n"
         ...
         *
         * @return true if it's an exception of that ProgrammingLanguage, and false if the exception should be handled
         *     elsewhere
         */
        protected abstract boolean handleLangException(ScriptException e, Entity ent, RunOutcome progress);
}
