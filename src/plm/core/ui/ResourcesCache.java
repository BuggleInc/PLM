...
public class ResourcesCache {
   ...
   */
  public static ImageIcon getIcon(String path) { return getIcon(path, false); }
  public static void setIcon(String path, ImageIcon icon) { iconsCache.put(path, icon); }
  /**
   * Lazy loading of ImageIcon resources.
   * @param path of the image resource to be loaded.
  ...
  {
    String name = basePath.getClass().getPackage().getName().replaceAll("\\.", "/") + "/" + path;
    return getIcon(name);
  }

  public static int getBusyIconsSize() { return ResourcesCache.busyIcons.length; }

  public static Icon getBusyIcons(int busyIconIndex) { return ResourcesCache.busyIcons[busyIconIndex]; }
  ...
  public static ImageIcon getStarredIcon(ImageIcon icon, Exercise exo)
  {
    ...
    if (!iconsCache.containsKey(path)) {
      ...
      if (exo.getProgLanguages().contains(Game.getInstance().programmingLanguageManager.LIGHTBOT)) {
        ...
        if (Game.getInstance().studentWork.getPassed(exo, Game.getInstance().programmingLanguageManager.LIGHTBOT))
        ...
        Graphics g = combined.getGraphics();
        g.drawImage(icon.getImage(), 0, 0, null);
        Dimension[] positions = new Dimension[] {new Dimension(26, 0), new Dimension(26, 16), new Dimension(42, 0), new Dimension(42, 16)};
        int curPos            = 0;
