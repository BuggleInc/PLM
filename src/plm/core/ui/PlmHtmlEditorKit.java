...
public class PlmHtmlEditorKit extends HTMLEditorKit {
  ...
  public static String filterHTML(String in, boolean showAll) { return filterHTML(in, showAll, false, null); }
  ...
  public static String filterHTML(String in, boolean showAll, boolean showMulti, ProgrammingLanguage[] currLang)
  {
    ...
    if (showAll) {
      ...
      for (ProgrammingLanguage lang : Game.getInstance().getProgrammingLanguageManager().langs) {
        ...
        res      = res.replaceAll("(?s)\\[!" + l + "\\](.*?)\\[/!\\]", "<font color=\"" + langColors.get(l) + "\">$1</font>");
      }
      for (ProgrammingLanguage lang : Game.getInstance().getProgrammingLanguageManager().langs) {
        String l = lang.getLang().toLowerCase();
      ...
      for (ProgrammingLanguage lang : Game.getInstance().getProgrammingLanguageManager().langs) {
        ...
        for (ProgrammingLanguage lang2 : Game.getInstance().getProgrammingLanguageManager().langs) {
          if (!lang2.equals(lang)) {
            String l2 = lang2.getLang().toLowerCase();
            for (ProgrammingLanguage lang3 : Game.getInstance().getProgrammingLanguageManager().langs) {
              if (!lang3.equals(lang) && !lang3.equals(lang2)) {
                String l3 = lang3.getLang().toLowerCase();
                res       = res.replaceAll("(?s)\\[!" + l + "\\|" + l2 + "\\|" + l3 + "\\](.*?)\\[/!\\]",
                                           "<font color=\"" + langColors.get(l + "|" + l2 + "|" + l3) + "\">$1</font>");
              }
            }
          }
        }
    ...
    if (showMulti) {
      ...
      while (!strtemp.equals(res)) {
        ...
        for (ProgrammingLanguage lang : Game.getInstance().getProgrammingLanguageManager().langs) {
          ...
          for (ProgrammingLanguage lang2 : Game.getInstance().getProgrammingLanguageManager().langs) {
            String l2 = lang2.getLang().toLowerCase();
            if (lang.equals(Game.getInstance().getProgrammingLanguage()) || lang2.equals(Game.getInstance().getProgrammingLanguage()))
  ...
  public PlmHtmlEditorKit() { baseExercise = null; }

  public PlmHtmlEditorKit(Lecture _baseExercise) { baseExercise = _baseExercise; }

  @Override public ViewFactory getViewFactory() { return new HTMLFactoryX(); }
  public static String getCSS()
  {
    String header = "  <style type=\"text/css\">\n"
                    + "    body { font-family: tahoma, \"Times New Roman\", serif; font-size:" + Game.getProperty(Game.PROP_FONT_SIZE, "10px", true) +
                    "; margin:10px; }\n"
...
class EmptyView extends IconView {
  ...
  public EmptyView(Element elem) { super(elem); }
  @Override public float getPreferredSpan(int axis) { return 0; }
...
class MyIconView extends View {
  ...
  public MyIconView(Element elem, Lecture baseExercise)
  {
    ...
    if (filename == null) {
      if (baseExercise == null)
        ...
        System.err.println(Game.i18n.tr("<img> tag without src attribute in exercise {0} at offset {1}", baseExercise.getName(), elem.getStartOffset()));
   ...
   */
  @Override public Shape modelToView(int pos, Shape a, Position.Bias b) throws BadLocationException
  {
