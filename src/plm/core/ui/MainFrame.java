...
public class MainFrame extends JFrame implements GameStateListener, GameListener, HumanLangChangesListener {
  ...
  private void initMenuBar(Game g)
  {
      ...
      @Override public void actionPerformed(ActionEvent e)
      {
        ...
        if (selectedFile != null) {
          if (selectedFile.exists()) {
            int dialogResult = JOptionPane.showConfirmDialog(null, Game.i18n.tr("Do you want to overwrite {0}?", selectedFile.getName()),
    ...
    miSessionImport = new JMenuItem(new ImportSession(g, i18n.tr("Read Session from disk"), null, this));
  ...
  public void appendToTitle(String addendum) { this.setTitle(MainFrame.frameTitle + "      " + addendum); }
  ...
  private void initToolBar(Game g)
  {
    ...
    debugButton = new PropagatingButton(new StepExecution(g, i18n.tr("Step"), ResourcesCache.getIcon("img/btn-debug.png")));
    // shortcut ctrl-b
    debugButton.setMnemonic(KeyEvent.VK_B);

    stopButton = new PropagatingButton(new StopExecution(g, i18n.tr("Stop"), ResourcesCache.getIcon("img/btn-stop.png")));
    // shortcut ctrl-s
    stopButton.setMnemonic(KeyEvent.VK_S);
    stopButton.setEnabled(false);

    resetButton = new PropagatingButton(new Reset(g, i18n.tr("Reset"), ResourcesCache.getIcon("img/btn-reset.png")));
    // shortcut ctrl-z
    resetButton.setMnemonic(KeyEvent.VK_Z);
    resetButton.setEnabled(true);

    demoButton = new PropagatingButton(new PlayDemo(g, i18n.tr("Demo"), ResourcesCache.getIcon("img/btn-demo.png")));
    // shortcut ctrl-d
  ...
  @Override public void stateChanged(Game.GameState type)
  {
    switch (type) {
      case LOADING:
      case SAVING:
    ...
    if (lecture instanceof Exercise) {
      ...
      for (ProgrammingLanguage l : exo.getProgLanguages()) {
        if (!g.programmingLanguageManager.isValidProgLanguage(l))
          System.err.println("Request to add the programming language '" + l + "' to exercise " + exo.getName() +
                             " ignored. Fix your exercise or upgrade your PLM.");
      }
    } else {
      hideWorldView();
    }
  ...
  @Override public void selectedWorldHasChanged(World w) { /* don't care */ }
