...
public class ZipSessionKit implements ISessionKit {
  ...
  public ZipSessionKit(Game game) { this.game = game; }

  private File openSaveFile(File path, Lesson lesson) { return new File(path, "plm-" + lesson.getId() + ".zip"); }

  @SuppressWarnings("unchecked") @Override public void storeAll(File path) throws UserAbortException
  {
  ...
  @Override public void loadAll(File path)
  {
  ...
  @Override public void cleanAll(File path)
  {
    for (Lesson lesson : this.game.getLessons())
      cleanLesson(path, lesson);
  }

  @Override public void storeLesson(File path, Lesson lesson) throws UserAbortException
  {
    ...
    try {
      ...
      int n            = JOptionPane.showOptionDialog(null,
                                                      Game.i18n.tr("The PLM were unable to save your session file for lesson {0} ({1}:{2}).\n\n"
                                                                       + " Would you like proceed anyway? Any solution typed so far will be lost.",
                                                                   lesson.getName(), ex.getClass().getSimpleName(), ex.getLocalizedMessage()),
                                                      Game.i18n.tr("Your changes are NOT saved."), JOptionPane.YES_NO_CANCEL_OPTION, JOptionPane.ERROR_MESSAGE, null,
                                                      options, options[1]);
      if (n == 1) {
  ...
  public void loadLesson(File path, Lesson lesson)
  {
    ...
    try {
      ...
      int n            = JOptionPane.showOptionDialog(null,
                                                      Game.i18n.tr("The PLM were unable to load your code for lesson {0} ({1}:{2}).\n\n"
                                                      ...
                                                      Game.i18n.tr("Error while loading your session"), JOptionPane.YES_NO_CANCEL_OPTION, JOptionPane.ERROR_MESSAGE, null,
                                                      options, options[1]);
      if (n == 1) {
  ...
  @Override public void cleanLesson(File path, Lesson lesson)
  {
    File saveFile = openSaveFile(path, lesson);
